...
$.ajaxSetup({
    ...
    complete: function(XMLHttpRequest, textStatus) {
        try {
            var data = null;
            try { data = XMLHttpRequest.responseJSON || JSON.parse(XMLHttpRequest.responseText || '{}'); } catch (e) {}
            if (data && (data.code == 1 || data.code == '1' || data.code == 401)) {
                var tip = data.msg || '登录超时，请重新登录';
                var goLogin = function() {
                    top.location.href = (typeof ctx !== 'undefined' ? ctx : '/') + 'login';
                };
                if ($.modal && typeof $.modal.alertWarning === 'function') {
